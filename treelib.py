...
#
# VICTOR SAYS: This Python file comes from http://compbio.mit.edu/pub/summon/summon-1.8.6/examples/
#
...
class MITTree:
    ...
    def copy(self):
        ...
        tree = MITTree(nextname = self.nextname)
        if self.root != None:
            # copy all nodes
...
#            try:
            ...
            if char1 == "(":
                ...
                # Victor: my addition, to add internal names
                node.name = token
                #print "node.name ", node.name
                for d in discovered_children_nodes:
                    #print "adding child ", d.name, " to parent ", node.name
                    self.addChild(node, d)
...
def subtree(tree, node):
    ...
    # make new tree
    tree2 = MITTree(nextname = tree.newName())
